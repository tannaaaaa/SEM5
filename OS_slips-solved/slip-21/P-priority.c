...
{
    char name[80] ;
    int at,bt,ft,wt,tat,tbt,priority;
}process ;
...
int getprocess()
{
    ...
    for(i=0;i<processcount;i++)
    {
        ...
        {
            if(p[i].priority < min) //compare priority
            {
                min = p[i].priority ;
                p1 = i ;
            }
...
void scheduleprocess()
{
    ...
    while(1)
    {
        ...
        else
        {
            ...
            {
                p[i].ft = currenttime ;
                p[i].tat = p[i].ft - p[i].at ;//finishtime-arrival time
...
void display()
{
    int i;
    printf( "\n--------------------------------------------------------------------------------\n") ;
    printf("process Arrival Burst Piority Finsih TurnAround Waiting\n");
    printf("--------------------------------------------------------------------------------\n");
    for(i=0 ; i<processcount ; i++)
        printf("%s\t %d\t%d\t %d\t %d\t %d\t%d\n",p[i].name,p[i].at,p[i].bt,p[i].priority,p[i].ft,p[i]. tat,p[i].wt) ;
    printf("--------------------------------------------------------------------------------") ;
